describe('Tambah Laporan Kerbau Jantan | Peternak', () => {
    ...
    it('edit laporan with valid data', async () => {
        ...
        await browser.url('http://127.0.0.1:8000/peternak/susu/1/edit');
        await pauseFor5Seconds();

        // Update the laporan with new data
        const inputTanggal = await $('[name="tanggal"]');
        await inputTanggal.setValue('2023-12-12'); // Update tanggal

        const inputJumlahSusu = await $('[name="jumlah_susu"]');
        await inputJumlahSusu.setValue('12 liter'); // Update jumlah_susu
